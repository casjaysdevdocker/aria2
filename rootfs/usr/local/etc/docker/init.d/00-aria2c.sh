...
# @@File             :  00-aria2c.sh
# @@Description      :  Init script for aria2c download manager + nginx/AriaNg frontend
...
if [ -n "$ARIA2C_ENABLED" ]; then
  if [ "$ARIA2C_ENABLED" != "yes" ]; then
...
RUN_DIR="/run/aria2"
# set log directory
LOG_DIR="/data/logs/aria2"
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the working dir
WORK_DIR=""
# - - - - - - - - - - - - - - - - - - - - - - - - -
# port which service is listening on (aria2c RPC port)
SERVICE_PORT="6800"
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
# wrapper: launches aria2c in background then execs nginx in foreground
EXEC_CMD_BIN='/usr/local/etc/docker/bin/start-aria2'
# command arguments
EXEC_CMD_ARGS=''
# execute script before
EXEC_PRE_SCRIPT=''
# Set to 'yes' so PID sentinel is tracked
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Set default type - [custom,sqlite,redis,postgres,mariadb,mysql,couchdb,mongodb,supabase]
...
# root user name
root_user_name="${ARIA2C_ROOT_USER_NAME:-}"
# root user password
root_user_pass="${ARIA2C_ROOT_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Normal user info [password/random]
# normal user name
user_name="${ARIA2C_USER_NAME:-}"
# normal user password
user_pass="${ARIA2C_USER_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Load variables from config
# Generated by my dockermgr script
if [ -f "/config/env/aria2c.script.sh" ]; then
  . "/config/env/aria2c.script.sh"
fi
# Overwrite the variables
if [ -f "/config/env/aria2c.sh" ]; then
  . "/config/env/aria2c.sh"
fi
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
__update_conf_files() {
  ...
  local get_data_dir
  local get_config
  local get_session_file
  local rpc_port="${RPC_PORT:-${RPC_LISTEN:-6800}}"
  local dht_port="$DHT_LISTEN_PORT"
  local secret_rpc_base64=""
  get_data_dir="$(grep -Rs 'dir=' "/config/aria2/aria2.conf" | awk -F'=' '{print $2}')"
  get_config="$(find "$WWW_ROOT_DIR/js" -name 'aria-ng-*.min.js' 2>/dev/null | grep -v 'f1dd57abb9.min' | head -n1 || true)"
  get_session_file="$(grep -Rs 'aria2.session' "/config/aria2/aria2.conf" | awk -F'=' '{print $2}' | head -n1)"
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # delete files
  #__rm ""
  ...
  #   secret_rpc_base64="$(printf '%s' "$RPC_SECRET" | base64 -w 0)"
